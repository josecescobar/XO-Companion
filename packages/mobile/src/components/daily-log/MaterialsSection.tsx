...
interface MaterialsSectionProps {
  ...
  editable?: boolean;
  onEdit?: (entry: MaterialEntry) => void;
  onDelete?: (entryId: string) => void;
  onAdd?: () => void;
}

export function MaterialsSection({ entries, editable, onEdit, onDelete, onAdd }: MaterialsSectionProps) {
      ...
      {entries.map((e) => (
        <View key={e.id} style={[styles.entry, { backgroundColor: colors.surfaceSecondary, borderColor: colors.border }]}>
          <View style={styles.row}>
            <Text style={[styles.name, { color: colors.text }]}>{e.material}</Text>
          ...
          <Text style={[styles.details, { color: colors.textSecondary }]}>{e.quantity} {e.unit} · {e.condition.replace(/_/g, ' ')}</Text>
          {e.supplier && <Text style={[styles.supplier, { color: colors.textSecondary }]}>Supplier: {e.supplier}</Text>}
        </View>
      ))}
      {editable && (
...
const styles = StyleSheet.create({
  ...
  supplier: { fontSize: 13, marginTop: 2 },
